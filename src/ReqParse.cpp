...
/*   Updated: 2026/04/23 16:47:20 by fmaurer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ReqParse::parseReqLine(t_RequestLine& rl, const str& rlstr)
{
  ...
  while (k <= MAX_TARGET_LEN && !std::isspace(rlstr[i + k]))
    k++;
  Logger::log_dbg0(
      "ReqParse: found this target URL: '" + rlstr.substr(i, i + k) + "'");
  if (k > MAX_TARGET_LEN)
    return HTTP_400;

  rl.target = rlstr.substr(i, i + k);
  ...
  if (i + k > MAX_REQLINE_LEN)
    return HTTP_400;
  Logger::log_dbg0(
      "ReqParse: found this httpVer: '" + rlstr.substr(i, i + k) + "'");
  rl.httpVersion = rlstr.substr(i, i + k);
  if (rlstr.compare(i + k, str::npos, CRLF) != 0)
    return HTTP_400;

  // Logger::log_dbg0("ReqParse: found this method: " +
  // method2str(rl.method));
